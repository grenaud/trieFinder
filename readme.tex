...
trieFinder is a software for the automatic alignment of tags stemming from Massively Parallel Signature Sequencing (MPSS) to preannotated databases of putative sequence tags from both the genome and transcriptome ( Refseq and Unigene ). Thus, the laborious process of aligning the tags and parsing the results to create a final summary for each tag is obviated. The core elements of trieFinder are written in C++ with high order scripts in Perl. Although the software was originaly written to deal with Zebrafish data, it can be easily accomodated for other species.  \\
...
$^2$ : Degenerate means matches with greater or equal than 1 mismatches.
